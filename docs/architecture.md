...
| [process](../src/process.rs) | 按 PID 读取原生进程身份和名称 | 独立核验进程身份，避免为查询名称刷新全部系统指标 |
...
| 类型 | 主要信息 | 作用 |
| --- | --- | --- |
| `ScanOptions` | 本地端口集合、是否只查监听 | 显式区分指定端口查询和监听列表 |
| `SocketInfo` | 协议、本地/远程端点、TCP 状态、所属进程、归属完整性 | 保留定位占用所需的上下文 |
| `ProcessInfo` | PID、可选名称、可选身份 | 保留已知信息，允许名称或身份不可读取 |
...
`ScanReport.complete` 表达在当前平台可见性范围内是否发现影响完整性的问题。它与每条记录的 `ownership` 分开：例如，一张协议表读取失败会使整个报告不完整，其他成功读取的端点仍可以有已核验的所属进程。字段定义见 [JSON 接口](json.md)。

## 平台采集

### Linux：协议表与 inode 关联

[Linux 后端](../src/scanner/linux.rs) 读取 `/proc/self/net/tcp`、`tcp6`、`udp`、`udp6`，保留地址、TCP 状态和套接字 inode。先按查询条件筛选，再为目标 inode 建立进程归属索引。
...
[Windows 后端](../src/scanner/windows.rs) 分别读取 IPv4/IPv6 的 TCP/UDP 系统表。TCP 使用包含全部状态的表类型，由公共查询规则决定是否只保留监听记录。原生字段提供地址、端口、状态、归属 PID 和 IPv6 scope。
